...
contract Swiss is Ownable, ReentrancyGuard {
    ...
    constructor(address _newGameController) Ownable(_newGameController) {
    ...
    function changeGameServiceFeeAddress(
        address newServiceFeeAddress
    ) public onlyOwner {
        serviceFeeAddress = newServiceFeeAddress;
    }

    function changeGameServicePercentage(
        uint8 newFeeServicePercentage
    ) public onlyOwner {
        require(
